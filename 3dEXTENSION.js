/**
 * Turbowarp 3D Extension - TurboWarp/Scratch Extension Format (fixed, RAW mode)
 * Просто разместите этот файл и загружайте по сырой ссылке.
 */
...
class ModelLoader {
  ...
  async loadFromZip(name, zipUrl) {
    ...
    try {
      ...
      if (!mainFile) throw new Error('No model found in ZIP');
...
// TurboWarp ищет вот именно это объявление!
window.turbowarp3d = async function(runtime) {
