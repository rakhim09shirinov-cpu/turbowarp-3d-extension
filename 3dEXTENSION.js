/**
 * Turbowarp 3D Extension - TurboWarp/Scratch Extension Format (fixed)
 * Совместимо с TurboWarp! Просто разместите этот файл и загружайте по ссылке.
 */
(function() {
  'use strict';
  ...
  const loadScript = (url) => {
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => {console.warn(`Could not load script: ${url}`); resolve();};
  ...
  class LODManager {
    ...
    setChunkSize(size) { this.chunkSize = size; }
  ...
  // ==== TurboWarp Extension Entry Point ====
  // TurboWarp вызывает эту функцию при загрузке расширения
  // runtime передают автоматически
  const TurboWarpExtension = async function(runtime) {
    ...
    return new Turbowarp3DExtension(runtime);
  };

  // Регистрация для TurboWarp
  if (typeof Scratch !== 'undefined' && Scratch.extensions && typeof Scratch.extensions.registerLoader === 'function') {
    Scratch.extensions.registerLoader('turbowarp3d', TurboWarpExtension);
  } else if (typeof window !== 'undefined') {
    window.turbowarp3d = TurboWarpExtension;
  }
})();
